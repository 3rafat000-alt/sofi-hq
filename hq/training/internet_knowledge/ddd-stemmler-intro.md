...
[ddd](https://khalilstemmler.com/articles/tags/ddd/)[typescript](https://khalilstemmler.com/articles/tags/type-script/)[software design](https://khalilstemmler.com/articles/tags/software-design/)
_Also from the**[Domain-Driven Design with TypeScript](https://khalilstemmler.com/articles/categories/domain-driven-design/)** article series_.
...
Eventually, the codebase had became so large that adding new features on top of it took _nearly 3x_ the amount of time it would have taken when I first started. 
Lack of encapsulation and object-oriented design were to blame.
I had an [Anemic Domain Model](https://khalilstemmler.com/wiki/anemic-domain-model).
...
## Modeling the Domain 
There are huge payoffs in domain modeling. When our code lines up with the real life domain, we end up with a [rich declarative design](https://khalilstemmler.com/articles/typescript-domain-driven-design/ddd-vs-crud-design/) that will enable us to make changes and add new features exponentially faster.
...
Domain-Driven Developers need to be comfortable with the following:
  ...
  * General Design Principles ([YAGNI](https://khalilstemmler.com/wiki/yagni), KISS, DRY)
...
**The software design roadmap** : Check out this [ roadmap](https://khalilstemmler.com/articles/software-design-architecture/full-stack-software-design/) I put together in order to figure out what you need to know in order to be most comfortable with Domain-Driven Design (which is right at the end of the roadmap).
...
See [here](https://khalilstemmler.com/articles/domain-driven-design-intro/#building-blocks) for the list of building blocks for DDD applications.
## Protecting the Domain Layer 
In order to do DDD well, we need to keep the [SOLID principles](https://khalilstemmler.com/articles/solid-principles/solid-typescript/) in mind, organize a central [domain layer](https://khalilstemmler.com/articles/enterprise-typescript-nodejs/clean-nodejs-architecture/) at the core of our [Layered Architecture](https://herbertograca.com/2017/08/03/layered-architecture/) and use a LOT of [dependency inversion & injection](https://khalilstemmler.com/articles/tutorials/dependency-injection-inversion-explained/) in order to connect interface adapters to persistence, web and external technologies. 
...
A large part of DDD is protecting the domain model by using a layered architecture. Check out [this article](https://khalilstemmler.com/articles/software-design-architecture/organizing-app-logic/) on what each layer is responsible for.
"The Clean Architecture". From the golden Uncle Bob archives. Also known as a Layered Architecture, Ports & Adapters, Hexigonal, etc.
...
The **most powerful part about aggregates is that they dispatch Domain Events** which can be used to _co-locate business logic in the appropriate subdomain_.
### Domain Services 
This is where we locate domain logic that doesn't belong to any one object conceptually.
Domain Services are most often executed by application layer [Application Services / Use Cases](https://khalilstemmler.com/articles/enterprise-typescript-nodejs/application-layer-use-cases/). Because Domain Services are a part of the **Domain Layer** and adhere to the [dependency rule](https://khalilstemmler.com/wiki/dependency-rule/), Domain Services aren't allowed to depend on infrastructure layer concerns like [Repositories](https://khalilstemmler.com/articles/typescript-domain-driven-design/repository-dto-mapper/) to get access to the domain entities that they interact with. Application Services fetch the necessary entities, then pass them to domain services to run allow them to interact.
We use repositories in order to retrieve domain objects from persistence technologies. Using software design principles like the [Liskov Subsitution Principle](https://khalilstemmler.com/articles/solid-principles/solid-typescript/) and a layered architecture, we can design this in a way so that we can easily make architecture decisions to switch between an in-memory repository for testing, a MySQL implementation for today, and a MongoDB based implementation 2 years from now.
### Factory 
...
We might also want to create domain objects from templates using the [prototype pattern](https://www.geeksforgeeks.org/prototype-design-pattern/) or through the use of an [abstract factory](https://khalilstemmler.com/wiki/abstract-factory).
...
Typically [when we're dealing with CRUD apps](https://khalilstemmler.com/articles/enterprise-typescript-nodejs/when-crud-mvc-isnt-enough/), we add new **domain logic** that we've identified by adding more `if/else` statements.
...
Depending on the project, it might be more worthwhile to continue building an [Anemic Domain Model](https://khalilstemmler.com/wiki/anemic-domain-model). Choosing DDD coincides with a lot of the [arguments I made for when it's right to use TypeScript over JavaScript](https://khalilstemmler.com/articles/when-to-use-typescript-guide/) for your project. Use DDD for #3 of the _3 Hard Software Problems_ : The Complex Domain Problem.
## Conclusion 
...
d) you read my article and you realized [you have an anemic domain model](https://khalilstemmler.com/wiki/anemic-domain-model) and you don't wish to have one.
The thing about Domain modeling is that it does take a little bit of time to start to get comfortable with. It can be a bit awkward to get accustomed to organizing your code this way, but when you start to reap the benefits of DDD, I think you'll naturally prefer to organize your backend code this way over the [Anemic Domain Model](https://khalilstemmler.com/wiki/anemic-domain-model) and [Transaction Script](https://martinfowler.com/eaaCatalog/transactionScript.html) approach.
_More in this series so far_..
[Understanding Domain Entities - DDD w/ TypeScript](https://khalilstemmler.com/articles/typescript-domain-driven-design/entities/)
[Value Objects - DDD w/ TypeScript](https://khalilstemmler.com/articles/typescript-value-object)
[Using UUIDs instead of Auto-Incremented Primary Keys](https://khalilstemmler.com/articles/auto-increment-or-uuid/)
...
[Intention Revealing Interfaces [w/ Examples] - Domain-Driven Design w/ TypeScript](https://khalilstemmler.com/articles/typescript-domain-driven-design/intention-revealing-interfaces/)
...
[Handling Collections in Aggregates (0-to-Many, Many-to-Many) - Domain-Driven Design w/ TypeScript](https://khalilstemmler.com/articles/typescript-domain-driven-design/one-to-many-performance/)
[Challenges in Aggregate Design #1 - Domain-Driven Design w/ TypeScript](https://khalilstemmler.com/articles/typescript-domain-driven-design/domain-modeling-1/)
...
[Where Do Domain Events Get Created? | Domain Driven Design w/ TypeScript](https://khalilstemmler.com/blogs/domain-driven-design/where-do-domain-events-get-dispatched/)
[Decoupling Logic with Domain Events [Guide] - Domain-Driven Design w/ TypeScript](https://khalilstemmler.com/articles/typescript-domain-driven-design/chain-business-logic-domain-events/)
  1. _See[this article](https://khalilstemmler.com/articles/enterprise-typescript-nodejs/when-crud-mvc-isnt-enough/) on how to know when MVC isn't enough_.
[↩](https://khalilstemmler.com/articles/domain-driven-design-intro/#fnref-1)


Not sure where you are on the path? [Take the Phases of Craftship quiz](https://khalilstemmler.com/phases-quiz) — 7 questions, personalized results.
### Stay in touch!
Join 20000+ value-creating Software Essentialists getting actionable advice on how to master what matters each week. 🖖
View more in [Domain-Driven Design](https://khalilstemmler.com/articles/categories/domain-driven-design)
© khalilstemmler • 2026
