...
> **⚡ Binding state 2026-09-05 (R3.1 + Audit-ALL-Phase3 — owner directive "اكمل كل المؤجل"):** the system runs **17 rooms · 121 agents · 113 skills**. The Data room (08) was merged into Architecture (04) in R3.1 — then re-split: 08 is now Localization (loc-*) and 16 is Innovation (inn-*) per Audit-ALL-Phase2+3 (owner directive). Historical amendment notes below (§2) quoting older counts (15 rooms · 114 agents · 109 skills) are **kept as history** (Law 13 continuity) — the derived counts above are the binding reality; they are guarded by `hq/core/tooling/registry_guard.py` + `count_sync.py` with **zero pending** warnings.
...
└── .opencode/             Source of the 121 agents and 113 skills (operating layer · gate-0 guarded by hq/core/tooling/registry_guard.py + count_sync.py)
```
